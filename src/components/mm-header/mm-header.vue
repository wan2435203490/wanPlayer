<template>
  ...
  <header class="mm-header">
    <h1 class="header">
      <a href="https://2fun.top" target="_blank">wanPlayer 在线音乐播放器</a>
      <img
        v-if="visitorBadge"
        :src="visitorBadge"
        alt="累计访问数"
    ...
    >
      <div class="mm-dialog-text">
        ...
        <p>2、点击页面右上角的“登录”</p>
        <p>3、点击您的头像，进入我的主页</p>
        <p>4、复制浏览器地址栏 /user/home?id= 后面的数字（wyy UID）</p>
      </div>
    </mm-dialog>
    <!--退出-->
  ...
  methods: {
    ...
    _getUserPlaylist(uid) {
      getUserPlaylist(uid).then(({ playlist = [] }) => {
        ...
        this.user = creator
        setTimeout(() => {
          this.$mmToast(`${this.user.nickname} 欢迎使用 wanPlayer`)
        }, 200)
      })
    },
    ...mapActions(['setUid'])
